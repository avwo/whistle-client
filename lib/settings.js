...
const parseSettings = (data) => {
  ...
  return {
    ...
    startAtLogin: getValue(data, 'startAtLogin') !== false,
  };
};
...
ipcMain.on('autoSetProxy', async (_, autoSetProxy) => {
  ...
  showToast(`Set as system proxy at start is ${autoSetProxy ? 'enabled' : 'disabled'}`);
});
ipcMain.on('startAtLogin', (_, startAtLogin) => {
  startAtLogin = !!startAtLogin;
  setStartAtLogin(startAtLogin);
  storage.setProperty('startAtLogin', startAtLogin);
  showToast(`Start at login is ${startAtLogin ? 'enabled' : 'disabled'}`);
});
