...
const parseSettings = (data) => {
  ...
  let pwd = getString(getValue(data, 'password'), 64);
  ...
  return {
    ...
    rejectUnauthorized: !!getValue(data, 'rejectUnauthorized'),
...
ipcMain.on('applySettings', async (_, data) => {
  ...
  const rejectChanged = curSettings.rejectUnauthorized !== data.rejectUnauthorized;
  if (!isRunning() || portChanged || hostChanged || rejectChanged
