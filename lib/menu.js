...
const getEnabledRules = () => {
  ...
  const list = rulesConfig.list.filter(rule => rule.selected);
  const result = [
    '',
    `Plugins: ${rulesConfig.pluginsDisabled ? 'OFF' : 'ON'}`,
    `Rules: ${rulesConfig.disabled ? 'OFF' : `ON (${list.length})`}`,
  ];
  list.forEach((item) => {
...
const updateTitle = (force) => {
  let title = getTitle();
  if (!force && title === curTitle) {
    return;
  }
  curTitle = title;
  title = `${title}${getEnabledRules()}`;
  const win = getWin();
  try {
    if (win) {
      win.setTitle(title);
...
const hideDockMenu = {
  ...
  icon: icons.uncheck,
  click() {
    const hide = storage.getProperty('hideFromDock');
      updateHideFromDock(!hide); // eslint-disable-line
  },
};
...
const updateHideFromDock = (hide) => {
  ...
  if (hide === undefined) {
    hide = storage.getProperty('hideFromDock');
  }
  hide = !!hide;
  if (hide) {
    app.dock.hide();
  ...
  if (updateCheckbox) {
    updateCheckbox(hideDockMenu, hide); // eslint-disable-line
    storage.setProperty('hideFromDock', hide);
  }
};
